...
function initializeApp(tgUser) {
    ...
    const referrerId = startParam || new URLSearchParams(window.location.search).get('ref');
    ...
    userRef.onSnapshot(async (doc) => {
        if (!doc.exists) {
            ...
            // --- FIX for old accounts missing name ---
            if (tgUser && (userState.username === "User" || !userState.telegramUsername)) {
